...
Class Sap_xml_handler {
	...
	function create_SAP_XML($order_id) {
		...
		while($order_user_info_data = $user_info_request->fetch()) {
			...
			} else if($order_user_info_data['address_type'] == 'ST') {
		...
		//STATES que pour les USA et chine
		if($state_id == 44 || $state_id == 223) {
		...
		//Find the language code to insert in SAP, if order language is Fr show 22 else show 3
		...
		//Génération du type de TVA applicable
		...
		//Génération de l'adresse de base
		...
		*/
		...
		if($order_user_info_data_ST['company'] != null && $order_user_info_data_ST['company'] != ''){
		...
		$XML_content.="<OB>\n\t<CLIENT>\n\t\t<BOM>\n\t\t\t<BO>\n";
			...
			$XML_content.="\t\t\t<BPAddresses>\n";
				...
				$XML_content.="\t\t\t\t<row>\n";
					...
					$XML_content.="\t\t\t\t\t<Country>".$country_data['country_2_code']."</Country>\n";
				$XML_content.="\t\t\t\t</row>\n";
		 	...
		 	$XML_content.="\t<COMMANDE>\n\t\t<BOM>\n\t\t\t<BO>\n";
				...
				$XML_content.="\t\t\t<Documents>\n";
					$XML_content.="\t\t\t\t<row>\n";
						...
						$XML_content.="\t\t\t\t\t<U_OB1CODELANGUE>".$langugeCodeSap."</U_OB1CODELANGUE>\n";
						$XML_content.="\t\t\t\t\t<U_OB1TYPETOTAL>".$U_OB1TYPETOTAL."</U_OB1TYPETOTAL>\n";
				...
				while($order_items_data = $order_items_request->fetch()) {
					$XML_content.="\t\t\t\t<row>\n";
						...
						if($order_data['order_status'] == 'D' && $order_items_data['product_item_price'] != $order_items_data['product_discountedPriceWithoutTax']) {
							$discountPercent = ( 1 - ( $order_items_data['product_discountedPriceWithoutTax'] / $order_items_data['product_item_price'] ) )*100;
							$XML_content.="\t\t\t\t\t<DiscountPercent>".$discountPercent."</DiscountPercent>\n";
						}
					$XML_content.="\t\t\t\t</row>\n";
				}
				$XML_content.="\t\t\t</Document_Lines>\n";
		...
		/*
		echo 'BUSINESSPARTNERS<br>';
		echo 'user_code : '.$user_data_BT['user_code'].'<br>';
		echo 'last_name+first_name : '.$user_data_BT['last_name'].' '.$user_data_BT['first_name'].'<br>';
		echo 'phone1 : '.$user_data_BT['phone_1'].'<br>';
		echo 'fax : '.$user_data_BT['fax'].'<br>';
		echo 'phone2 : '.$user_data_BT['phone_2'].'<br>';
		echo 'email : '.$user_email.'<br>';
		echo 'FederalTaxID : '.$FederalTaxID.'<br>';
		echo 'codelangue : '.$codelangue.'<br>';
		echo 'U_OB1TYPETOTAL : '.$U_OB1TYPETOTAL.'<br>';
		echo 'VatLiable : '.$VatLiable.'<br>';
		echo 'VatGroup : '.$VatGroup.'<br>';
		echo 'last_name : '.$user_data_BT['last_name'].'<br>';
		echo 'exonereTVA : '.$exonereTVA.'<br>';
		echo 'PayTermsGrpCode : '.$PayTermsGrpCode.'<br>';
		echo '<br><br>';

		echo 'CONTACTEMPLOYEE<br>';
		echo 'last_name : '.$order_user_info_delivery['last_name'].'<br>';
		echo 'first_name : '.$order_user_info_delivery['first_name'].'<br>';
		echo 'phone_1 : '.$order_user_info_delivery['phone_1'].'<br>';
		echo 'fax : '.$order_user_info_delivery['fax'].'<br>';
		echo 'phone_2 : '.$order_user_info_delivery['phone_2'].'<br>';
		echo 'user_email : '.$user_email.'<br>';
		echo '<br><br>';

		echo 'BPADDDRESSES-SHIP<br>';
		echo 'AddressName : '.$adressnameST.'<br>';
		echo 'Street : '.$order_user_info_delivery['address_1'].'<br>';
		echo 'Block : '.$order_user_info_delivery['address_2'].'<br>';
		echo 'ZipCode : '.$order_user_info_delivery['zip'].'<br>';
		echo 'City : '.$order_user_info_delivery['city'].'<br>';
		echo 'State : '.$state_data['state_3_code'].'<br>';
		echo 'Country : '.$country_data['country_2_code'].'<br>';
		echo '<br><br>';

		echo 'BPADDDRESSES-BILL<br>';
		echo 'AddressName : '.$adressnameBT.'<br>';
		echo 'Street : '.$order_user_info_data_BT['address_1'].'<br>';
		echo 'Block : '.$order_user_info_data_BT['address_2'].'<br>';
		echo 'ZipCode : '.$order_user_info_data_BT['zip'].'<br>';
		echo 'City : '.$order_user_info_data_BT['city'].'<br>';
		echo 'State : '.$state_data['state_3_code'].'<br>';
		echo 'Country : '.$country_data['country_2_code'].'<br>';
		echo '<br><br>';

		echo 'DOCUMENT-ITEM<br>';
		echo 'DocDate : '.date("Ymd", strtotime($order_data['created_on'])).'<br>';
		echo 'DocDueDate : '.date("Ymd", strtotime($order_data['created_on'])).'<br>';
		echo 'CardCode : '.$user_data_BT['user_code'].'<br>';
		echo 'NumAtCard : '.$order_id.'<br>';
		echo 'ShipToCode : '.$adressnameST.'<br>';
		echo 'PayToCode : '.$adressnameBT.'<br>';
		echo 'U_OB1CODELANGUE : '.$countryCode[$LanguageCode].'<br>';
		echo 'U_OB1TYPETOTAL : '.$U_OB1TYPETOTAL.'<br>';
		echo '<br><br>';

		echo 'LISTRE PRODUIT<br>';
		$order_items_request = $db_sap->query('SELECT * 
										FROM h8q2p_virtuemart_order_items
										WHERE virtuemart_order_id = '.$order_id);
		while($order_items_data = $order_items_request->fetch()) {
			echo 'order_item_sku : '.$order_items_data['order_item_sku'].'<br>';
			echo 'product_quantity : '.$order_items_data['product_quantity'].'<br>';
			echo 'product_item_price : '.round($order_items_data['product_item_price'],2).'<br>';
		}
		echo '<br><br>';

		echo "ExpenseCode : ".$ExpenseCode.'<br>';
		echo "order_shipment : ".round($order_data['order_shipment'],2).'<br>';
		echo '<br><br>';
		echo 'group id '.$user_data_BT['virtuemart_shoppergroup_id'].'<br>';
		*/
